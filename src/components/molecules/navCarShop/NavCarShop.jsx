...
const NavCarShop = ({ className }) => {
  ...
  return (
    <div className={`carShop ${className}`}>
      {cart.map((item, index) => (
        <div className='carShop__item' key={item.currentImg}>
          ...
          <div className='carShop__quantityItem'>
            <input className='carShop__input' type="number" value={item.quantity} readOnly />
            <div className='carShop__quantityController'>
              <Icon altIcon={'minusIcon'} onClick={() => handleDecrement(index)} />
              <Icon altIcon={'plusIcon'} onClick={() => handleIncrement(index)} />
            </div>
          </div>
          <Icon
      ...
      {cart.length === 0 ? (
        <p className='carShop__emp'>Empty Cart</p>
      ) : null}
      {cart.length > 0 && (
        <Button
